...
const reducer = (state: AppState, action: Action): AppState => {
  switch (action.type) {
    ...
    case GET_TASKLIST:
      return {
        ...
        taskLists: state.taskLists.some((wl) => wl.id === action.payload.id)
          ? state.taskLists.map((wl) => (wl.id === action.payload.id ? action.payload : wl))
    ...
    case UPDATE_TASKLIST:
      return {
        ...state,
        taskLists: state.taskLists.map((wl) => (wl.id === action.payload.id ? action.payload : wl)),
      };
    ...
    case GET_TASK: {
      ...
      const taskExists = existingTasks.some((task) => task.id === action.payload.task.id);

      // Either update existing task or add new one
      const updatedTasks = taskExists
        ? existingTasks.map((task) => (task.id === action.payload.task.id ? action.payload.task : task))
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const api: AppContextType["api"] = {
    fetchTaskLists: async () => {
      const response = await axios.get<TaskList[]>("/api/task-lists", jsonHeaders);
      dispatch({ type: FETCH_TASKLISTS, payload: response.data });
    },
    getTaskList: async (id: string) => {
      const response = await axios.get<TaskList>(`/api/task-lists/${id}`, jsonHeaders);
      dispatch({ type: GET_TASKLIST, payload: response.data });
    },
    createTaskList: async (taskList) => {
      const response = await axios.post<TaskList>("/api/task-lists", taskList, jsonHeaders);
      dispatch({ type: CREATE_TASKLIST, payload: response.data });
    },
    getTask: async (taskListId: string, taskId: string) => {
      const response = await axios.get<Task>(`/api/task-lists/${taskListId}/tasks/${taskId}`, jsonHeaders);
      dispatch({
        type: GET_TASK,
    ...
    updateTaskList: async (id, taskList) => {
      const response = await axios.put<TaskList>(`/api/task-lists/${id}`, taskList, jsonHeaders);
    ...
    fetchTasks: async (taskListId) => {
      const response = await axios.get<Task[]>(`/api/task-lists/${taskListId}/tasks`, jsonHeaders);
      dispatch({
        type: FETCH_TASKS,
    ...
    createTask: async (taskListId, task) => {
      const response = await axios.post<Task>(`/api/task-lists/${taskListId}/tasks`, task, jsonHeaders);
      dispatch({
        type: CREATE_TASK,
        payload: { taskListId, task: response.data },
      });
    ...
    updateTask: async (taskListId, taskId, task) => {
      const response = await axios.put<Task>(`/api/task-lists/${taskListId}/tasks/${taskId}`, task, jsonHeaders);
      dispatch({
        type: UPDATE_TASK,
